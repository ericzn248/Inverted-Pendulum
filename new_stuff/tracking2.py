...
def getPoint(HSV,BOUNDS,color=RED):
    global points
    mask = getMask(HSV,BOUNDS,color)
...
def getInputs(HSV,BOUNDS,cartcolor,prev_vals,dt):
    
    p1,p2,angle = prev_vals
    p1_t = getPoint(HSV,BOUNDS,cartcolor)
...
while __name__ == "__main__":
  ...
  while True:
      ...
      if xx%50==0:
        try:
          trans = getTransM(HSV,BOUNDS,BLUE)
        except Exception as e:
          print(e)

      try:
        toprint, vals, prev = getInputs(HSV,BOUNDS,RED,prev,t-prev_t)
      except Exception as e:
        pass
      # img = cv2.resize(img,(1000,562))
      img = cv2.putText(img,toprint,org,font,fscale,(0,0,0),thickness)
      # cv2.imshow('red',getMask(HSV,BOUNDS,BLUE))
      cv2.imshow('img',img)
      cv2.waitKey(1)
      xx += 1
